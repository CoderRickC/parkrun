...
- `parkrun_athlete.py` — Richard's full results history (athlete **448437**).
  Runs on the Raspberry Pi via `run_weekly.sh` + cron: daily at 22:00 (catches
  events run abroad), plus hourly 11:00-16:00 on Saturdays, Christmas Day and
  New Year's Day. It commits and pushes `data/` only when a new parkrun appears,
  so only one run per parkrun ever commits and a quiet week commits nothing.
  See `SETUP.md`.
